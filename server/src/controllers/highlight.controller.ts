...
import { createHighlightSchema, updateHighlightSchema } from "./achievement.schema";
...
export const createHighlight = async (req: AuthenticatedRequest, res: Response) => {
  const validated = createHighlightSchema.parse(req.body);
...
export const updateHighlightByID = async (req: AuthenticatedRequest, res: Response) => {
  const validated = updateHighlightSchema.parse(req.body);
...
export const deleteHighlightByID = async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user || !req.user.userID) throw new ApiError(UNAUTHORIZED, "unauthorized to perform this action");
  const { userID } = req.user;
